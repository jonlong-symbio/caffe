...
void Col2imLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void Col2imLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < kernel_shape_.shape(0); ++i) {
    kernel_total_size *= kernel_shape_data[i];
  }
  DCHECK_EQ(bottom[0]->shape(channel_axis_) % kernel_total_size, 0);
  top_shape.push_back(bottom[0]->shape(channel_axis_) / kernel_total_size);
  for (int i = 0; i < num_spatial_axes_; ++i) {
...
template <typename Dtype>
void Col2imLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const Dtype* bottom_data = bottom[0]->cpu_data();
  Dtype* top_data = top[0]->mutable_cpu_data();
  for (int n = 0; n < num_; ++n) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      col2im_cpu(bottom_data + n * bottom_dim_, channels_,
      ...
      col2im_nd_cpu(bottom_data + n * bottom_dim_, num_spatial_axes_,
...
template <typename Dtype>
void Col2imLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      im2col_cpu(top_diff + n * top_dim_, channels_,
          top[0]->shape(channel_axis_ + 1),
          top[0]->shape(channel_axis_ + 2),
          kernel_shape_.cpu_data()[0], kernel_shape_.cpu_data()[1],
          pad_.cpu_data()[0], pad_.cpu_data()[1],
      ...
      im2col_nd_cpu(top_diff + n * top_dim_, num_spatial_axes_,
          top[0]->shape().data() + channel_axis_,
          bottom[0]->shape().data() + channel_axis_,
          kernel_shape_.cpu_data(), pad_.cpu_data(), stride_.cpu_data(),
...
#ifdef CPU_ONLY
STUB_GPU(Col2imLayer);
#endif

INSTANTIATE_CLASS(Col2imLayer);
REGISTER_LAYER_CLASS(Col2im);
